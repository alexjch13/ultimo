$(document).ready(function () {
  ...
  // permiso para notificaciones no se implemento
  ...
  function ChatListView(event, data) {
    ...
    for(var i = 0; i < pubnub.subscriptions.length; i++) {
      var chatName = pubnub.subscriptions[i],
		  ...
		  chatEl = $("<li><a href='#chatPage' data-channel-name='" + chatName + "'>" + chatName + "</a><a href='#delete' data-rel='dialog' data-channel-name='" + chatName + "'></a></li>");
  ...
  function ChatView(event, data) {
    ...
    // cambia el titulo del departamento no se implemento
  ...
  // Tmuestra nuevos mensajes en el chat
  ChatView.prototype.handleMessage = function (message, animate) {
    ...
    if (isBlurred) {
      ...
      // notificaciones no se implemento
  ...
  // inicialmente se inicia desactivado en la pagina principal
